...
class KnowledgeRetrieval:
    ...
    def load_data(self, local=True, file_path="/home/mandeep/Pictures/corpus/uploads/repository_generated.txt", url="https://mltblog.com/3y8MXq5", append=True):
        ...
        self.create_keyword_map(update_memory=True, save_to_file=True)
    ...
    def rephrase_with_mistral(self, description, use_api=True, use_cache=True):  # use_cache parameter kept for compatibility
        ...
        # Check cache first if enabled (currently disabled)
        # if use_cache and description in self._rephrase_cache:
        #     print("Using cached rephrasing result")
        #     return self._rephrase_cache[description]
        ...
        if not use_api:
            ...
            # Cache disabled
            # if use_cache:
            #     self._rephrase_cache[description] = result
            return result
        ...
        try:
            ...
            # Always use the small model for faster processing
            model = "mistral-small-latest"  # Always use small model
            client = Mistral(api_key=api_key)

            # No timeout parameter is used (not supported in this version)

            # Create a prompt for rephrasing as a chatbot response
            ...
            try:
                # Remove the timeout parameter as it's not supported in your version
                ...
                # Cache the result if caching is enabled (currently disabled)
                # if use_cache:
                #     self._rephrase_cache[description] = rephrased_description
                ...
                print(f"API call error: {e}. Using local rephrasing.")
                return self._local_rephrase(description)
    ...
    def save_backend_tables(self):
        ...
        # Create and save the keyword map
        self.create_keyword_map(update_memory=True, save_to_file=True)
